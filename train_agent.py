...
def train_agent(restore_prior_from='data/Prior.ckpt',
    ...
    for step in range(n_steps):
        ...
        new_experience = zip(smiles, score, score1, score2, prior_likelihood)
        ...
        # step_score2[0].append(step + 1)
        # step_score2[1].append(np.mean(score2))
        ...
        # logger.log(np.array(step_score1), "Scores1")
        # logger.log(np.array(step_score2), "Scores2")
    ...
    # seqs3 = Agent.sample(256)
    # seqs4 = Agent.sample(256)
    prior_likelihood, _ = Prior.likelihood(Variable(seqs))
    prior_likelihood = prior_likelihood.data.cpu().numpy()
    smiles = seq_to_smiles(seqs, voc)
    score, score1, score2 = scoring_function(smiles)
    ...
    #     f.write("SMILES Score Score1 Score2 PriorLogP\n")
    #     for smiles in zip(smiles1):
    #         f.write("{} {:5.2f} \n".format(smiles))
    #     for smiles in zip(smiles2):
    #         f.write("{} {:5.2f} \n".format(smiles))
    #     # for smiles in zip(smiles3):
    #     #     f.write("{} {:5.2f} \n".format(smiles))
    #     # for smiles in zip(smiles4):
    #     #     f.write("{} {:5.2f} \n".format(smiles))
    #     # for smiles in zip(smiles5):
    #     #     f.write("{} {:5.2f} \n".format(smiles))
