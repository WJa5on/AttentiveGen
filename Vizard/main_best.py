...
# score_source = ColumnDataSource(data=dict(x=[], y=[], y_mean=[], y1_mean=[], y2_mean=[]))
...
layout = layout([img_fig], sizing_mode="fixed")
curdoc().add_root(layout)

def update():
    f=open("memory")
    m=[]
    m=[i for i in f.readlines()][1:]
    mols = []
    scores = []
    for line in m:
        if len(scores) <=5:
    ...
    img_fig.text = '<h2>Best Generated Molecules</h2>' + '<div class="img_inside">' + img + '</div>'
    #score_source.data = dict(x=score[0], y=score[1], y_mean=running_average(score[1], 50), y1_mean=running_average(score[2], 50), y2_mean=running_average(score[3], 50))
